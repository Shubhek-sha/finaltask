    ...
    <main className="mx-auto max-w-4xl space-y-10 p-10">
      <header className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <Avatar fallback={user?.name.slice(0, 2).toUpperCase() ?? "?"} alt={user?.name ?? ""} />
      ...
      <Card>
        ...
        <CardFooter>
          <Can permissions={role?.permissions} I="project:write">
      ...
      <Can permissions={role?.permissions} I="admin:access">
        <Card>
          <CardHeader>
            <CardTitle>Admin panel</CardTitle>
            <CardDescription>Only visible to roles with admin:access.</CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-text-muted">
              Signed in as {user?.email} — organization {user?.organizationId}.
            </p>
          </CardContent>
        </Card>
      </Can>
